...
void deriveClosure(int startfile = 0,
			int endfile = 1,
			int mc = 0,
			int vec = 0,
			...
			std::string out = "test.root")
{
  ...
  for(int ifile = startfile; ifile<endfile; ++ifile){
	...
	h_avgA[ptBinCorr][etaBin]->Fill(0.5*(L2->relcor(pt_F[pJet],eta_F[pJet],1,0)-L2->relcor(pt_F[rJet],eta_F[rJet],1,0))/avgPtCorr);
    h_avgB[ptBinMPF][etaBin]->Fill(1 + missEtJets.Et()*cos(dphi2)/L2->mpfcor(pt_F[rJet],eta_F[rJet],1,0));
      }
    } //close entries
    fin->Close();
	}
...
{ 
  deriveClosure(atoi(argv[1]),atoi(argv[2]),atoi(argv[3]),atoi(argv[4]),argv[5],argv[6]);
  return 0;
}
