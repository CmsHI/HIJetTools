...
void deriveDijetResponse(int startfile=0, int endfile=1, int isMC=1){
  ...
  TFile *fout;
  if(isMC) fout = new TFile(Form("relDijetResponse_MC_%d.root",endfile),"recreate");
  else fout = new TFile(Form("relDijetResponse_%d.root",endfile),"recreate");
  fout->cd();
  for(int i=0; i<nbins_pt; i++){
